...
export function AcademicStructureView() {
  ...
  return (
    <div className="page-frame animate-fade">
      ...
      {loading ? (
        <div className="panel p-8 text-center text-teal-700">Loading Academic Structure from Supabase...</div>
      ) : (
                ...
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-xl border flex items-center justify-between transition-all ${
                    selectedYearId === year.id
                      ? 'bg-teal-50 border-teal-500/30 text-teal-700 font-semibold'
              ...
              {filteredSections.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {filteredSections.map((sec) => (
                    <div key={sec.id} className="p-4 rounded-xl bg-white border border-slate-200 flex flex-col justify-between">
                      <div className="flex items-center justify-between mb-3">
                        <span className="font-bold text-slate-900 text-lg">{sec.name}</span>
                        <span className="chip chip-teal text-xs">Active</span>
                      </div>
                      <div className="text-xs text-slate-500 space-y-1">
                        <p>Structure: {activeYearObj?.name} → {activeDeptObj?.code}</p>
                        <p className="text-teal-700/90 font-mono">Capacity: {sec.capacity || 60} students</p>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      {showSecModal && (
        <div className="drawer-backdrop" onClick={() => setShowSecModal(false)}>
          <div className="modal-card p-6 bg-slate-50 border border-slate-200 rounded-2xl max-w-md w-full" onClick={(e) => e.stopPropagation()}>
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold text-slate-900">Add Section to {activeDeptObj?.code}</h2>
          ...
          <table className="w-full text-left text-sm text-slate-700">
            ...
            <tbody className="divide-y divide-slate-800/60">
              {facultyAssignments.map((fac) => (
                <tr key={fac.id} className="hover:bg-slate-100 transition-colors">
                  <td className="p-3.5">
      ...
      <div className="panel">
        ...
        <div className="space-y-3">
          {[
            ...
            <div key={student.id} className="p-4 rounded-xl bg-white border border-slate-200 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-teal-50 border border-teal-500/30 flex items-center justify-center font-bold text-teal-700 text-sm">
      ...
      <div className="panel">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {[
            ...
            <div key={fac.email} className="p-4 rounded-xl bg-white border border-slate-200 flex items-center justify-between">
              <div>
                <strong className="text-slate-900 block">{fac.name}</strong>
                <span className="text-xs text-slate-500">{fac.title} · {fac.dept}</span>
